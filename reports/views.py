...
def client_stats(request):
    ...
    # Répartition des réservations par source (si applicable - à implémenter dans un système réel)
    ...
    if 'save_report' in request.POST:
        try:
            ...
            {', '.join([f"{client.profile.user.username}: {client.reservations_count} réservations" for client in top_clients_by_reservations[:5]])}
            
            Top clients par montant dépensé:
            {', '.join([f"{client.profile.user.username}: {client.total_spent} €" for client in top_clients_by_spending[:5]])}
            """
    ...
    if 'export_csv' in request.POST:
        ...
        for client in all_clients:
            writer.writerow([
                f"{client.profile.user.first_name} {client.profile.user.last_name}",
